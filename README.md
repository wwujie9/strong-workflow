...
小编辑器会展示模拟命中预览，区分当前优先级下新增命中的隐患，以及未来调高优先级可能影响的原规则和责任人
规则优先级管理支持拖拽、上移和下移排序；保存前会对当前隐患做全量模拟，展示规则命中和责任人是否变化
模板变更记录会追踪新增规则、编辑规则和调整优先级，记录操作人、时间、变更摘要和影响明细，随项目配置包导入导出
```
